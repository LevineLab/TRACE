function pop_evo = create_sim_evolved(N_mod, all_traits_num, evolved_traits_change, evo_corr_cell_sorted, evolved_trait_stats)


pop_evo = zeros(N_mod,length(all_traits_num));
traits_already_done = [];
%traits_already_done(1) = trait_to_change;
for i=1:length(evo_corr_cell_sorted)

    %current_pair = strsplit(evo_corr_cell_sorted{i,1}, '-');

    %tmp_trait_a = str2double( regexp(current_pair{1},'t(\d+)','tokens','once') );
    %tmp_trait_b = str2double( regexp(current_pair{2},'t(\d+)','tokens','once') );
    tmp_trait_a = evo_corr_cell_sorted(i,1);
    tmp_trait_b = evo_corr_cell_sorted(i,2);
    ...
    if (evo_corr_cell_sorted(i,4) < 0.05)

        if (trait_a_check == 1 && trait_b_check == 0)

            ta_rand = pop_evo(:,tmp_trait_a); %pull already calculated trait values

            %Linear regression from empirical data to generate correlated trait
            tmp_a = evolved_traits_change(:,tmp_trait_a); 
            tmp_b = evolved_traits_change(:,tmp_trait_b); 
            ...
            pop_evo(:,tmp_trait_b) = tb_rand; %pop_evoulate appropriate column with randomized trait values
        ...
        elseif (trait_a_check == 0 && trait_b_check == 1) %if trait a is uknown but trait b is known
            ...
            tb_rand = pop_evo(:,tmp_trait_b); %pull already calculated trait values
            ...
            pop_evo(:,tmp_trait_a) = ta_rand; %pop_evoulate appropriate column with randomized trait values
        ...
        elseif (trait_a_check == 0 && trait_b_check == 0) %if trait a is uknown but trait b is known
            ...
            mean_a = evolved_trait_stats(tmp_trait_a,1);
            sd_a = evolved_trait_stats(tmp_trait_a,2);
            ta_rand = sd_a.*randn(N_mod,1) + mean_a;
            pop_evo(:,tmp_trait_a) = ta_rand;
    ...
    elseif (evo_corr_cell_sorted(i,4) >= 0.05) %generate random numbers if significant correlation doesnt exist

        
        if (trait_a_check == 1 && trait_b_check == 0)

            mean_b = evolved_trait_stats(tmp_trait_b,1);
            sd_b = evolved_trait_stats(tmp_trait_b,2);
